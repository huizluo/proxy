...
func NewTunnelServer() provider.Provider {
	return &TunnelServer{
		cfg:    TunnelServerArgs{},
...
func (s *TunnelServer) GetName() string{
	return s.name
}
...
func (s *TunnelServer) GetConn() (conn2 net.Conn, err error) {
	var _conn tls.Conn
...
func (s *TunnelServer) UDPConnDeamon() {
	go func() {
		...
		RETRY:
				...
				for {
					...
					if err != nil {
						...
						go func(outConn net.Conn) {
							...
							for {
								srcAddrFromConn, body, err := conn.ReadUDPPacket(&outConn)
			...
			writer.Write(conn.UDPPacket(item.srcAddr.String(), *item.packet))
			err := writer.Flush()
			if err != nil {
				outConn = nil
