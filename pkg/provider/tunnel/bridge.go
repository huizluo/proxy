...
func NewTunnelBridge() provider.Provider {
	return &TunnelBridge{
		cfg: TunnelBridgeArgs{},
...
func (s *TunnelBridge) GetName()string {
	return s.name
}
...
func (s *TunnelBridge) ClientControlConn(inConn *net.Conn, key string) {
	...
	conn.CloseConn(item.ClientControl)
...
func (s *TunnelBridge) ChnDeamon(item *BridgeItem) {
	go func() {
		...
		OUT:
			...
			conn.IoBind(*serverConn, *clientConn, func(isSrcErr bool, err error) {
				conn.CloseConn(serverConn)
				conn.CloseConn(clientConn)
